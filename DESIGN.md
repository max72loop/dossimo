...
- **404 globale** : une carte flottante sur le fond crème associe une illustration
  abstraite de page introuvable (icône Lucide, sans actif supplémentaire) à une
  copie courte et deux sorties : accueil en action principale, dossiers en action
  secondaire. Le code `404` reste visible comme repère, mais le rouge sémantique
  n'est pas employé : une URL absente est une impasse de navigation, pas une panne.
...
- **Admin** ([`admin/layout.tsx`](src/app/admin/layout.tsx)) : en-tête encre, puis
  une **barre de navigation** sur fond `papier` ([`nav-admin.tsx`](src/components/admin/nav-admin.tsx))
  qui liste toutes les consoles en trois rubriques séparées par un filet, entrée
  courante en `bg-encre` + `aria-current`. La liste vient de
  [`sections.ts`](src/components/admin/sections.ts), partagée avec le sommaire.
  Chaque console ouvre par [`EnTeteConsole`](src/components/admin/en-tete-console.tsx)
  (`max-w-5xl`, `h1` serif `text-2xl`, aide en `text-ardoise`) et ne porte **aucun
  lien de retour** : la barre s'en charge. L'avertissement « modifie la production »
  n'apparaît que sur les consoles qui écrivent ce que le produit lit (règles, devis),
  via `AvertissementProduction`. Le sommaire affiche par carte les **chiffres du
  jour** en mono `tabular-nums`, `text-avertissement` quand il y a quelque chose à
  faire ; un chiffre inconnu s'écrit « — », jamais 0.
...
- [x] **Maquettes Grand Slam** : deux pistes non publiées vivent sur `/mockups/grand-slam`, sans lien dans la navigation. Elles assument désormais une inspiration éditoriale imprimée beaucoup plus directe : grille de filets persistante, titres serif monumentaux et italiques, aplats pleine largeur, collage de cartes et séquences de données « programme » ; les cartes de promesse empruntent la silhouette perforée d’un billet. Les couleurs, polices, contraste, cibles tactiles et rayons restent ceux de Dossimo : ni palette concurrente, ni image, ni typographie de la référence ne sont repris. La piste A met l’accent sur la rapidité et le geste unique (« une photo du devis ») ; la piste B sur la protection de la prime et l’alternative au mandataire. Les engagements « 24 h » et « accepté ou remboursé » y sont explicitement qualifiés de promesses candidates : aucune promesse contractuelle n’est publiée ni reliée au tunnel avant validation opérationnelle, juridique et produit (2026-09-09).
...
| 2026-09-07 | **Maillage SEO et auteur incarné.** (1) Le cluster `/refus` entre dans la navigation : lien « Dossier refusé ? » au footer (colonne Ressources), dans la barre d'en-tête (visible dès `lg`, la barre est pleine à `md`) et en section propre du menu persistant (hub + les deux pages ciblées). (2) Les guides remplacent la liste « tous vers tous » de fin d'article par 3 ou 4 voisins de la même famille + un lien vers le pilier `/guides` ; les pages motifs gagnent un bloc « Éviter ce motif » vers le guide de prévention correspondant (`lib/refus/prevention.ts`). (3) Les guides sont signés « Max Landry, fondateur de Dossimo » (lien vers `/a-propos#auteur`, JSON-LD `Person` relié à l'`Organization`) au lieu d'« équipe éditoriale Dossimo ». Les 7 pages gelées de `seo/hermes.md` gardent leur rendu d'origine jusqu'au 2026-10-02 (`lib/seo/gel-seo.ts`). Aucun nouveau token. | Décision de Max du 2026-09-07. Le cluster refus (plus d'un tiers des URLs) n'était découvert que par le sitemap et n'héritait d'aucune autorité interne ; le maillage « tous vers tous » aplatissait le signal entre familles ; et sur un sujet réglementaire et financier, un site sans auteur identifiable part avec un handicap E-E-A-T face à Effy ou Hellio. **Ce point (3) revient sur les décisions des 2026-07-22 et 2026-07-27** qui cantonnaient l'identité personnelle aux pages légales : seuls le nom, le rôle et l'e-mail de contact (déjà publics dans les mentions légales) apparaissent ; adresse et SIREN restent sur les surfaces légales. |
...
| 2026-09-12 | Landing : suppression des **surtitres de section** (`SectionLabel`, mono capitales + point coloré) au-dessus de chaque titre, hero compris. Le composant est retiré de `page.tsx`, les titres reprennent la première ligne de leur bloc. Aucun token touché. | Ces mini-titres (« Comment ça marche », « La différence Dossimo », « Votre prochain dossier »…) donnaient à la page un aspect générique, très « gabarit IA ». Les titres serif portent seuls la structure ; l'ancre de navigation (`id`) reste sur la section. |
| 2026-09-13 | **Admin, navigation** : barre de navigation commune sous l'en-tête (toutes les consoles, trois rubriques : prospection, pilotage, produit et données), gabarit `EnTeteConsole` unique (largeur `max-w-5xl`, titre `text-2xl`), suppression de tous les liens de retour ad hoc. Le bandeau jaune permanent du layout disparaît : l'avertissement de production ne s'affiche plus que sur Règles et Devis ; le compte connecté passe dans l'en-tête. Le sommaire `/admin` porte les chiffres du jour par console. Les deux consoles du sprint bicanal sont retirées, leur tableau « par source utm » rejoint le Tunnel. Aucun token touché. | Dix consoles sans barre, dont deux mortes, et `/admin/regles` faisant hub par accident : le sommaire était inatteignable depuis huit pages sur dix. Un avertissement affiché sur toutes les pages, y compris celles qui ne font que lire, ne se lit plus. |
