...
| 2026-09-10 | Landing, bloc estimation : la **phrase de définition passe avant la mention obligatoire**, qui reste inchangée au mot près. Le paragraphe (`src/app/page.tsx`, section `Estimation`) s'ouvre désormais sur « Dossimo prépare et contrôle les dossiers MaPrimeRénov' et CEE des artisans RGE avant dépôt. » Aucun token, aucune couleur, aucune structure touchée. | La mention légale était la première phrase littéralement descriptive après celle du simulateur, seulement séparée par une illustration : les moteurs recollaient les deux et publiaient « Dossimo est un service indépendant **de simulation** et d'aide à la préparation de dossiers d'aide à la rénovation énergétique », perdant au passage la cible (artisans RGE), les deux dispositifs nommés et le contrôle de conformité, qui est le cœur de valeur (CLAUDE.md §4). La mention obligatoire n'est pas un résumé du produit ; elle ne doit pas se retrouver seule à en tenir lieu. Mesuré sur la page servie : « conformité » 1 occurrence, « anti-refus » 0, « artisan » 3 sur 8082 caractères. |
| 2026-09-10 | Balisage JSON-LD de la vitrine : le nœud **`Service` est désormais émis en toutes circonstances** (`src/app/page.tsx`, `JsonLd`), seul `offers` reste conditionné à la grille tarifaire ; il gagne `description`, `serviceType` et `audience: Artisans RGE`. L'`Organization` (`src/lib/seo/site.ts`) gagne `knowsAbout` (MaPrimeRénov', CEE, conformité, artisans RGE) et `areaServed: France`. **`sameAs` et `legalName` volontairement absents.** | Le `Service` vivait entièrement dans un `grille ? … : []` : quand la grille ne remontait pas de la base, le seul nœud qui dit ce que Dossimo FAIT disparaissait du balisage et il ne restait qu'Organization + WebSite + FAQPage. Le prix, lui, reste conditionnel (« ne jamais inventer un chiffre », AGENTS.md). `knowsAbout` rattache l'entité à son domaine : « Dossimo » est un nom disputé (variété de tomate, cheval de course, société britannique homonyme) et rien n'indiquait de quel Dossimo il s'agit. `sameAs` serait le rattachement le plus fort mais exige des profils publics qui n'existent pas : des URL inventées seraient pires que rien. `legalName` reste exclu par la décision du 2026-07-22 (aucune donnée nominative dans le JSON-LD). |
| 2026-09-10 | Landing, hero : le paragraphe sous le H1 nomme désormais la cible et les deux dispositifs — « Vous êtes artisan RGE. Envoyez le devis : … prépare le pack MaPrimeRénov' ou CEE complet. » Les verbes, le rythme en trois temps et le H1 sont inchangés ; seuls des noms sont ajoutés. Aucun titre réécrit, aucun token. | « MaPrimeRénov' & CEE · artisans RGE » n'existait que dans le `SectionLabel` au-dessus du H1, qui rend un `<p>` : pour une machine, du texte courant sans poids. Le premier paragraphe après le H1 est au contraire l'un des blocs les plus pondérés à l'extraction, et il décrivait le geste sans jamais nommer ni le dispositif ni la cible. Reste ouvert, et relevant d'une décision de marque : aucun des dix H1/H2 de la page ne nomme la catégorie. |
