-- Répare les objets de 0030_securite_comptes.sql, enregistrée comme appliquée
-- dans l'historique distant alors que son contenu n'a jamais été exécuté :
-- ni la table `auth_rate_limits` ni la fonction `consume_auth_rate_limit`
-- n'existaient en base. La RPC échouait donc à chaque appel, et comme
-- `consumeAuthRateLimit` échoue en mode fermé, toute connexion était refusée
-- avec « Trop de tentatives » dès la première tentative.
--
-- Rejoue le contenu de 0030 à l'identique. Tout y est idempotent, la migration
-- est donc sans effet sur une base où ces objets existent déjà.
...
-- Force PostgREST à recharger son cache de schéma pour exposer la RPC
-- immédiatement, sans attendre le rechargement périodique.
