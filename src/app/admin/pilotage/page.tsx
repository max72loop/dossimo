...
export default async function PilotagePage() {
  ...
  return <main className="mx-auto max-w-4xl px-8 py-10">
    ...
    <section className="mt-8 rounded border border-filigrane bg-blanc-casse p-5"><h2 className="font-serif text-lg font-semibold text-encre">Résultat des dépôts</h2><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">{Object.entries(comptes).map(([s, n]) => <Metric key={s} label={label[s]} value={n} />)}</div></section>
...
function Metric({ label, value }: { label: string; value: string | number }) { return <div className="rounded border border-filigrane bg-papier/40 p-4"><p className="text-xs text-ardoise">{label}</p><p className="mt-1 font-serif text-2xl font-semibold text-encre">{value}</p></div>; }
function List({ title, items, empty }: { title: string; items: [string, number][]; empty: string }) { return <section className="rounded border border-filigrane bg-blanc-casse p-5"><h2 className="font-serif text-lg font-semibold text-encre">{title}</h2>{items.length ? <ul className="mt-3 divide-y divide-filigrane">{items.map(([name,n]) => <li key={name} className="flex justify-between py-2 text-sm"><span className="text-encre">{name}</span><span className="font-mono text-ardoise">{n}</span></li>)}</ul> : <p className="mt-3 text-sm text-ardoise">{empty}</p>}</section>; }
