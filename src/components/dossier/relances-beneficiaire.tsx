...
export function RelancesBeneficiaire({ dossierId, attendues, uploads, enabled }: { dossierId: string; attendues: PieceAttendue[]; uploads: Upload[]; enabled: boolean }) {
  ...
  return <section className="mb-6 rounded border border-filigrane bg-blanc-casse p-5 shadow-sm">
    ...
    <div className="mt-4 border-t border-filigrane pt-4"><button type="button" disabled={busy} onClick={prepare} className="rounded border border-tampon/30 px-3 py-2 text-sm font-medium text-tampon">Préparer une relance à copier</button>{message && <div className="mt-3 rounded bg-papier/60 p-3"><p className="text-xs font-medium text-encre">Objet : {message.subject}</p><pre className="mt-2 whitespace-pre-wrap font-sans text-xs text-ardoise">{message.body}</pre><button type="button" onClick={() => navigator.clipboard.writeText(`Objet : ${message.subject}\n\n${message.body}`)} className="mt-3 text-xs font-medium text-tampon underline">Copier le message</button></div>}</div>
  </section>;
}
