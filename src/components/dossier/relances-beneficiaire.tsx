...
import { preparerRelanceManuelle, revoirPieceBeneficiaire } from "@/lib/reminders/actions";
...
export function RelancesBeneficiaire({ dossierId, attendues, uploads }: { dossierId: string; attendues: PieceAttendue[]; uploads: Upload[] }) {
  ...
  return <section className="mb-6 rounded border border-filigrane bg-blanc-casse p-5 shadow-sm">
    <div>
      ...
      <p className="mt-1 max-w-2xl text-sm text-ardoise">
        Les relances automatiques arrivent bientôt. En attendant, préparez le message à envoyer à votre client.
      </p>
    </div>
    ...
    <div className="mt-4 border-t border-filigrane pt-4"><button type="button" disabled={busy} onClick={prepare} className="rounded border border-tampon/30 px-3 py-2 text-sm font-medium text-tampon">Préparer le message</button>{message && <div className="mt-3 rounded bg-papier/60 p-3"><p className="text-xs font-medium text-encre">Objet : {message.subject}</p><pre className="mt-2 whitespace-pre-wrap font-sans text-xs text-ardoise">{message.body}</pre><div className="mt-3 flex flex-wrap gap-3"><a href={`https://wa.me/?text=${encodeURIComponent(message.body)}`} target="_blank" rel="noreferrer" className="text-xs font-semibold text-tampon underline">Envoyer par WhatsApp</a><a href={`sms:?&body=${encodeURIComponent(message.body)}`} className="text-xs font-semibold text-tampon underline">Envoyer par SMS</a><button type="button" onClick={() => navigator.clipboard.writeText(`Objet : ${message.subject}\n\n${message.body}`)} className="text-xs font-medium text-tampon underline">Copier le message</button></div></div>}</div>
  </section>;
}
